...
def check_response(response):
    ...
    if not response.status_code >= 200 < 300:
        click.echo("An error occured, http return code {}".format(response.status_code))
        exit()
    return response
...
def api_call(ctx, url, method="get", user_hash={}):
    ...
    if method.lower() not in methods:
        ...
        exit(1)
    auth = HTTPBasicAuth(ctx.obj["username"], ctx.obj["passwd"])
    # GET requests
    if method.lower() == "get":
...
@click.group()
@click.pass_context
@click.option("--user", "-u", help="Name of user to create")
@click.option(
    "--passwd", "-p", default=None, help="Password for user, leave blank for random"
)
@click.option("--ssh_key", "-s", default=None, help="Public ssh_key to upload")
@click.option(
    "--expiration", "-e", default=None, help="Password for user, leave blank for random"
)
@click.option("--email", "-m", default=None, help="Email address for user")
@click.option("--admin", "-a", is_flag=True, help="Use -a to create an admin user")
...
def delete(ctx):
    ...
    url = "{}/users/{}".format(ctx.obj["server_url"], username)
